...
async function loginUser(req, res) {
  let { account, password, lang, provider } = req.body;
  ...
  if (!account || (!password && provider == "password")) {
  ...
  try {
    const user = await User.findOne(queryAndType.searchQuery).select(
      "_id email firstName password lang providers"
    );
    ...
    if (!user.providers.includes(provider)) {
      return ResponseHandler(res, "auth", 420, lang);
    }
    ...
    if (
      ...
      (((await bcrypt.compare(password || "", user.password)) &&
        provider == "password") ||
        (user.providers.includes(provider) && provider !== "password"))
    ) {
